...
LoadFromFolder::LoadFromFolder(const rclcpp::NodeOptions &options)
    : Node("load_from_folder_node", options), count_(0),
...
void LoadFromFolder::on_timer() {
  // Load image
  cv::Mat image =
      cv::imread(files_.at(count_ % files_.size()), cv::IMREAD_COLOR);
  ...
  msg->id = ++count_;
  RCLCPP_INFO(this->get_logger(), "Publishing frame: %u / %lu", msg->id,
              files_.size());
  msg->image = im_msg;
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    vslam_components::data_loader_nodes::LoadFromFolder)