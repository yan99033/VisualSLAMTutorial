...
I will provide a one-year commitment to support the community, hoping the framework will incorporate more features and improve through optimization and fixing bugs.

\section{Framework}

To promote flexibility, we can organize the visual SLAM components as individual modules/libraries to test them in isolation before integrating them into the framework. 
We create the modules as composable nodes to run them in a single process.
...
\begin{itemize}
  ...
  \item \textbf{vslam\_node} processes the incoming Frame messages to recover the camera motion and structure of the scene. It consists of six plugins: 
    \begin{itemize}
      ...
      \item \textbf{back\_end\_plugin} optimizes the tracking constraints and loop constraints.
      \item \textbf{visualization\_plugin} gets the frame messages and updates the visualizer accordingly.
    \end{itemize}
...
\begin{itemize}
  ...
  \begin{lstlisting}
    ...
    #       map.addFramePairConstraint(frame_pair)
    #       
...
\begin{itemize}
  ...
  \begin{lstlisting}
    ...
    addFramePairConstraint(FramePair)

    # Get N previous keyframes
    # Algorithm:
    # kf_it $\leftarrow$ frames.find(curr_kf)
    # kfs = vector<Frame*>
    # if kf_it == frames.end() # cannot be found
    #   return kfs
    # count = 0
    # Go one step back
    # kf_it--
    # while count < n and kf_it != frames.begin():
    #   # skip the frame pairs that have
    #   if frame_pairs.find(kf_it.first, curr_kf->id) != frame_pairs.end()
    #     continue
    #   kfs.push_back(kf_it.second.get())
    #   count++
    # return kfs
    getNPreviousUnconstrainedKeyframes(unsigned int n)

    # Current keyframe getter
    # Algorithm:
    # throw an error if curr_kf is a nullptr
    # return curr_kf
    curr_kf $\leftarrow$ getCurrentKeyframe()

    # Current keyframe setter
    # Verify if the frame is valid 
    #   pointer is not a nullptr and its frame exists
    setCurrentKeyframe(Frame*)

    # --- private --- 
    # Current keyframe closest to the current camera pose to the best
    #   of map's knowledge
    # Frame* curr_kf
    ...
    map<long unsigned, shared_ptr<Frame>> frames
    # Ensure the first value in the pair is always smaller than the second
    map<pair<long unsigned, long unsigned>, FramePoseConstraint> frame_pairs;
