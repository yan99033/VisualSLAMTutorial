...
For example, A visual SLAM system consists of camera tracking, mapping, loop closing, and visualization components. 
...
First, we want to define a good way to connect the visual SLAM components. 
...
Specifically, each component in the framework can be defined as a plugin, which can be customized or replaced for enhanced performance, so long as the interfaces (the inputs and outputs) are unchanged.
